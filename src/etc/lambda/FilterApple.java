...
public class FilterApple {
    ...
    /*
        @solution - try1: 사과바구니에서 녹색 사과만 필터링하기
        @problem - 만약 다른 색깔의 사과를 필터링 해야 한다면
                   새로운 메서드를 선언해야 한다
    ...
    /*
        ...
        @problem - 만약에 필터 기준이 색상이 아닌 무게라면
     */
    ...
    /*
        @solution - try3: 동작을 파라미터화 하면 해결된다
                        ...
                        - 자바는 함수를 전달할 수 있는가
                            ...
                            -> 함수 개념이 아닌 객체에 소속된 메서드 개념
                        - 전달되는 함수가 고정되어 있는가
                            -> 필터 조건에 따라 함수 내용이 달라야 한다
                            -> 그러면 어떻게 함수 이름이 통일되면서 다른 내용을 전달할까
                            -> 오버라이딩 (상속, 인터페이스)
     */
    ...
    public static <T> List<T> filter(List<T> basket, GenericPredicate<T> predicate) {
        // 필터링 된 객체만 담을 바구니
        List<T> filteredBasket = new ArrayList<T>();
        // 반복문을 통한 필터링
        for (T t : basket) {
            if (predicate.test(t)) {
